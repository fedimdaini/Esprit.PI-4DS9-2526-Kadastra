...
// ── Valid type/transaction combinations ──────────────────────────────────────
    ...
    <div style={{ height: '100%', position: 'relative', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          >
            <option value="appartement">{t('map.apartment')}</option>
            <option value="maison">{t('map.house')}</option>
            <option value="terrain">{t('map.land')}</option>
            <option value="local">{t('map.commercial')}</option>
          </select>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          >
            {validTransactions.map(tx => (
              <option key={tx} value={tx}>{tx === 'vente' ? t('map.sale') : t('map.rental')}</option>
            ))}
          </select>
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ width: 100, height: 8, background: 'linear-gradient(90deg, #ef4444, #facc15, #22c55e)', borderRadius: 4 }}/>
          <span style={{ fontSize: 10, opacity: 0.8 }}>+30%</span>
        </div>
        <div style={{ display: 'flex', gap: 14, marginTop: 2, fontSize: 10, opacity: 0.75 }}>
          <span>{t('map.decline')}</span>
          <span>{t('map.stable')}</span>
          <span>{t('map.growth')}</span>
        </div>
        <div style={{ marginTop: 2, fontSize: 10, opacity: 0.6 }}>
          ∗ = {t('map.estimated')} &nbsp;|&nbsp; taille ∝ amplitude
        </div>
      </div>
      ...
      >
        ...
        {Object.entries(GOVERNORATE_COORDS).map(([gov, coords]) => {
          ...
          const label      = trend_pct > 3 ? t('map.growth') : trend_pct < -3 ? t('map.decline') : t('map.stable');
              ...
              >
                <div style={{ minWidth: 160, fontFamily: 'system-ui,sans-serif' }}>
                  <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 4, color: '#1e293b' }}>
                    {gov} {estimated && <span style={{ fontSize: 10, color: '#64748b' }}>({t('map.estimated')})</span>}
                  </div>
                  <div style={{ color: trendColor, fontWeight: 700, fontSize: 13, marginBottom: 4 }}>
                    {label} {pctStr}
                  </div>
                  <div style={{ fontSize: 11, color: '#475569', lineHeight: 1.7 }}>
                    <div>📍 {t('map.current')} : <strong>{fmtPrice(current_price_m2)}/m²</strong></div>
                    <div>🔮 {t('map.forecast12m')} : <strong>{fmtPrice(future_price_m2)}/m²</strong></div>
                    {estimated && (
              ...
              <Popup minWidth={200}>
                <div style={{ fontFamily: 'system-ui,sans-serif', padding: '2px 0' }}>
                  ...
                  <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                    <tbody>
                      <tr>
                        <td style={{ color: '#64748b', paddingBottom: 3 }}>{t('map.current')}</td>
                        <td style={{ fontWeight: 700, textAlign: 'right' }}>{fmtPrice(current_price_m2)}/m²</td>
                      </tr>
                      <tr>
                        <td style={{ color: '#64748b' }}>{t('map.forecast12m')}</td>
