  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, width: '100%' }}>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>
        {t('pagination.page')} <span style={{ color: 'var(--primary)' }}>{current}</span> {t('pagination.of')} {totalPages}{' '}
        <span style={{ opacity: 0.3, margin: '0 8px' }}>|</span>
        {t('pagination.total')} <span style={{ color: 'var(--primary)' }}>{total.toLocaleString()}</span> {t('pagination.assets')}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface-alt)', padding: '6px 12px', borderRadius: 16, border: '1px solid var(--border)' }}>
        <NavButton direction="prev" disabled={current === 1} onClick={() => current > 1 && onChange(current - 1)} label={t('pagination.previous')} />
        ...
        <NavButton direction="next" disabled={current === totalPages} onClick={() => current < totalPages && onChange(current + 1)} label={t('pagination.next')} />
      </div>
    </div>
  );
}
